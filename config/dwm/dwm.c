...
#define ISVSIBLE(C)            ((C->tags & C->mon->tagset[C->mon->seltags]))
...
drawbar(Monitor *m)
{
	...
	x = 0;
	for (i = 0; i < LENGTH(tags); i++) {   //LENGTH(tags)
		w = TEXTW(tags[i]);
		wdelta = selmon->alttag ? abs(TEXTW(tags[i]) - TEXTW(tagsalt[i])) / 2 : 0;
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagsSel : SchemeTagsNorm]); //tagscolr
		//drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i); original
		drw_text(drw, x, 0, w, bh, wdelta + lrpad / 2, (selmon->alttag ? togglealttags[i] : tags[i]), urg & 1 << i);
...
//				urg & 1 << i);
	...
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0); //layout 

	if ((w = m->ww - tw - x) > bh) {
/*		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeInfoSel : SchemeInfoNorm]);
			drw_text(drw, x, 0, w - 2 * sp, bh, lrpad / 2, m->sel->name, 0);
			drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0);  //ApplicationStatus
			if (m->sel->isfloating)
